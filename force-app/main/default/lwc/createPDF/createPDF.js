...
export default class PdfGenerator extends LightningElement {
	...
	handleGeneratePDF() {
		if (this.jsPDFInitialized) {
			...
			doc.text("Name of Contractor: "+this.Timesheet.dbt__Employee__r.Name, 10, 10);
			doc.addImage(pic, 190, 2, 15, 15);
			doc.text("Digital Biz Tech", 148, 10);
			doc.text("Fortnight Ending: "+this.Timesheet.dbt__End_Date__c, 10, 20);
			doc.text("Name of Manager: "+this.Timesheet.dbt__Manager__r.Name, 10, 30);
			doc.text("Manager Email: "+this.Timesheet.dbt__Manager__r.Email, 10, 40)
			doc.text("Billable Hours: "+this.Timesheet.dbt__Billable_Hours__c, 10, 50);
			doc.text("Total Hours: "+this.Timesheet.dbt__Total_Hours__c, 10, 60);
